...
import { useVideoProctoring } from "../../Proctoring/hooks/useVideoProctoring";
...
const DynamicExam = () => {
  ...
  const handleStartProctoring = async () => {
    ...
    if (cameraStarted) {
      ...
      setCameraError("Camera access is required to start or resume the exam. Please allow camera permission.");
      return false;
    }
  };
  ...
  const startExam = async () => {
    try {
      // Require camera permission before starting
      const ok = await handleStartProctoring();
      if (!ok) {
        // Do not start exam if camera is not allowed
        return;
      }
  ...
  const returnToFullScreen = async () => {
    try {
      ...
      // Require camera permission before resuming
      const ok = await handleStartProctoring();
      if (!ok) {
        return;
      }
